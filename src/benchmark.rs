...
use crate::{
    backend::create_configured_deriver,
    domain::{
        BackendConfiguration, BackendKind, CandidateBatch, CandidateCursor, SearchPhase,
        SecretMnemonic, VerificationTarget,
    },
    error::RecoverError,
    search::RecoveryPlan,
};
...
pub struct SustainedBenchmarkConfig {
    ...
    /// Typed runtime configuration applied to the backend
...
pub struct SustainedBenchmarkResult {
    ...
    /// CPU percentage assigned to an explicit hybrid backend
...
pub fn benchmark_sustained_pipeline(
    ...
    let mut deriver = create_configured_deriver(backend, mnemonic, config.configuration)?;
...
mod tests {
    ...
    fn rejects_a_plan_too_small_for_sustained_measurement() {
        ...
        let config = SustainedBenchmarkConfig {
            ...
            configuration: BackendConfiguration::cpu(4).unwrap(),
